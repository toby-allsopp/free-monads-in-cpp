...
struct fmap_compiles_t<T, void_t<decltype(fmap(id, T<int>{}))>>
    : std::true_type {};
template <template <typename> class T>
...
namespace Functor {
  ...
  static_assert(!IsFunctor<Nothing>,
                "specialisation with no members is not a Functor");
...
#ifndef _MSC_VER
  static_assert(fmap_compiles<Correct>,
                "fmap on correct specialisation compiles");
#endif
